...
async function getAllClassesSessionsStudents(req, res) {
    try {
        ...
        const students = await Student.find()
            .select('-TC -__v') // legacy safe cleanup
            .populate({
                path: 'classId',
                populate: {
                    path: 'sessionId',
...
async function createClass(req, res) {
    try {
        ...
        await Class.create({ className, sessionId });
...
async function updateClass(req, res) {
    try {
        ...
        if (className) classObj.className = className.trim().toUpperCase();
        if (sessionId) classObj.sessionId = sessionId;
...
async function createSession(req, res) {
    try {
        await Session.create({ year: req.body.year });
        res.redirect('/admin/tc');
    } catch (error) {
        res.status(500).json({ message: error.message });
    }
...
async function createStudent(req, res) {
    try {
        ...
        const tcKey = await uploadToR2(buffer, file.mimetype);
...
async function viewStudentTC(req, res) {
    try {
        ...
        res.setHeader(
            'Content-Disposition',
            `inline; filename="TC-${student.student_name}"`
        );
